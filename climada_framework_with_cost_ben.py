...
sys.path.append(os.path.join(HOME_DIR, 'climada_netcdf/climada_python'))
...
# customer number of years if needed
...
def apply_measure(ent, exp, imp, impf_set, hazard, meas_set, meas):
    #  Add to entity
    ent.measures = meas_set
  
    # Apply measure
    new_exp, new_impf_set, new_haz = meas.apply(exp, impf_set, hazard)
    axes = new_impf_set.plot()
    plt.savefig(OUT_DIR + f'impact_func_post_measure.png', dpi=500)
    plt.close()
    impf_set.plot()
    plt.savefig(OUT_DIR + f'impact_func_pre_measure.png', dpi=500)
    plt.close()

    # Plot modified impact info:
    ent.new_exp = new_exp
    ent.new_impf_set = new_impf_set
    imp_new = Impact()
    imp_new.calc(ent.new_exp, ent.new_impf_set, new_haz, save_mat='True')
    # impact before
    ax = imp.calc_freq_curve().plot(label='original')
    # impact after
    imp_new.calc_freq_curve().plot(axis=ax, label='measure')
    plt.legend()
    plt.savefig(OUT_DIR + f'exceddance_freq.png', dpi=500)
    plt.close()
...
def main():
    ...
    for i_ens_mem in ens_mem:
        ...
        if check_years_ge_15(nc): # check hazard data contains at least 15 days
            ...
            ## Define an adaptation measure(s)
            meas_1 = Measure()
            meas_1.name = 'trees'
            meas_1.haz_type = 'Heatstress'
            meas_1.color_rgb = np.array([0.2, 0.2, 0.7])
            meas_1.cost = 3000000
            meas_1.hazard_int_impact = (1, 0)
            meas_1.mdd_impact = (1, 0)
            meas_1.paa_impact = (0.75, 0) # 75% of assets affected
            
            meas_2 = Measure()
            meas_2.name = 'siesta'
            meas_2.haz_type = 'Heatstress'
            meas_2.color_rgb = np.array([0.2, 0.2, 0.7])
            meas_2.cost = 1000000
            meas_2.hazard_int_impact = (1, 0)
            meas_2.mdd_impact = (0.8, 0) # alter impact curve?
            meas_2.paa_impact = (1, 0)
            ...
            # apply adapation measure
            apply_measure(ent, exp, imp, impf_set, hazard, meas_set, meas_1)
            
            ## Cost-Benefit analysis
            # First add discount rate to entity
            ref_year = 2020
            future_year = 2041
            nyears = (future_year - ref_year)+1
            print(nyears)
            year_range = np.arange(ref_year, future_year+1)
            print(year_range)
            annual_discount_zero = np.zeros(nyears)
            discount_zero = DiscRates(year_range, annual_discount_zero)
            ent.disc_rates=discount_zero

            costben_measures_only = CostBenefit()
            costben_measures_only.calc(hazard, ent, 
                    haz_future=None, ent_future=None, future_year=2041,
                    risk_func=risk_aai_agg, imp_time_depen=None, save_imp=True)

            # Combine measures
            combined_costben = costben_measures_only.combine_measures(['trees', 'siesta'],
                                                          'Combined measures',
                                                          new_color=np.array([0.1, 0.8, 0.8]),
                                                          disc_rates=discount_zero)
            
            ## Cost-Benefit analysis with changing hazard & exposure
            # reference hazard
            nc_path, nc = read_hazard(warming_level='current', ens_mem=i_ens_mem)
            haz_present = define_hazard(nc_path,nc,variable,haz_type,custom_nyears=True)
            # reference exposure
            if exp_nc:
                exp_ref = read_exposures_nc(exposure_path, exp_var, 2020)
            else:
                exp_ref = read_exposure_csv(exposure_path)
            exp_inst_ref = exposure_instance(exp_ref, exp_unit)
            exp_inst_ref.ref_year = 2020

            # build entity
            entity_present = Entity(exposures=exp_ref, disc_rates=discount_zero,
                        impact_func_set=impf_set, measure_set=meas_set)

            costben = CostBenefit()
            costben.calc(haz_present, entity_present, haz_future=hazard, ent_future=ent,
            future_year=ssp_year, risk_func=risk_aai_agg, imp_time_depen=1, save_imp=True)

            def waterfall():
                return costben.plot_waterfall(haz_present, entity_present, hazard, ent,
                                risk_func=risk_aai_agg)

            ax = waterfall()
            plt.savefig(OUT_DIR + 'waterfall.png', dpi=500)
